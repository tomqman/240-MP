    ...
    , m_logFilePath(QDir::tempPath() + "/240mp-mpv.log")
{
    ...
    if (f.open(QFile::WriteOnly | QFile::Text)) {
        ...
        bool headless = detectHeadlessMode();
        if (headless) {
            // evdev key names used when running without X11/Wayland
            f.write("KEY_PLAYPAUSE cycle pause\n");
            f.write("KEY_PLAY cycle pause\n");
            f.write("KEY_PAUSE cycle pause\n");
            f.write("KEY_STOPCD stop\n");
            f.write("KEY_STOP stop\n");
            f.write("KEY_NEXTSONG playlist-next\n");
            f.write("KEY_NEXT playlist-next\n");
            f.write("KEY_PREVIOUSSONG playlist-prev\n");
            f.write("KEY_PREVIOUS playlist-prev\n");
            f.write("KEY_VOLUMEUP add volume 5\n");
            f.write("KEY_VOLUMEDOWN add volume -5\n");
            f.write("KEY_MUTE cycle mute\n");
        } else {
            // X11/Wayland key names
